...
class ImgQuDataset(Dataset):
    ...
    def simple_item_getter(self, idx):
        ...
        out = {
            'img': img,#resized image
            'idxs': torch.tensor(idx).long(),
            'qvec': torch.from_numpy(q_chosen_emb_vecs), #padded one but until phrase length.
            'qlens': torch.tensor(qlen), #number of words in query
            'annot': torch.from_numpy(target).float(), #x1,y1 -1 - 1
            'orig_annot': torch.tensor(annot).float(), #x1 y1 in original image non resized
...
#Comment Rohit: refclef is for referit
def get_data(cfg,ds_name='refclef'):
